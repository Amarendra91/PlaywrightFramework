...
test('Save login session storage.', async ({ page, homePage, cryptoUtil }) => {
  const decryptedUserName = cryptoUtil.decryptData(process.env.USER_NAME!);
  const decryptedPassword = cryptoUtil.decryptData(process.env.PASSWORD!);
